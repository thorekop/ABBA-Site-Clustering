		        ...
		        do
		            ...
		            do
			            ...
			            do
			                ...
			                old_log=../log/msprime/high_ils/pop_size_${pop_size}_div_time_${divergence_time}_rec_rate_${rec_rate}_mut_rate_${mut_rate}_intr_rate_${intr_rate}_P2_rate_${P2_rate}_r${r}.log

			                # Get the seed from the old log file.
			                seed=`cat ${old_log} | grep seed | cut -d " " -f 2`

			                # Set output file.
			                prefix=../res/msprime/high_ils/pop_size_${pop_size}_div_time_${divergence_time}_rec_rate_${rec_rate}_mut_rate_${mut_rate}_intr_rate_${intr_rate}_P2_rate_${P2_rate}_r${r}
			
			                # Submit slurm script.
			                if [ ! -f ${prefix}.tracts.txt ]
			                then
				                rm -f ${log}
				                sbatch --account ${acct} -o ${log} simulate_data_high_ils_tracts.slurm ${prefix} ${pop_size} ${divergence_time} ${rec_rate} ${mut_rate} ${intr_rate} ${P2_rate} ${seed}
			                fi
			            done
		            done
		        done
