...
iqtree <- read.delim("iqtree.txt")
iqtree$div_time = as.factor(iqtree$div_time)

pop_size = 1e5; mut_rate = 2e-9
rec_rate = 1e-8; align_length = 1000


paste0("IQTREE_pvalue_boxplots_pop_size_",pop_size, "_mut_rate_",mut_rate, "_rec_rate_", rec_rate, "_align_length_", align_length, ".pdf")



pdf(paste0("IQTREE_pvalue_boxplots_pop_size_",pop_size, "_mut_rate_",mut_rate, "_rec_rate_", rec_rate, "_align_length_", align_length, ".pdf"), width = 8, height =8)
...
for (k in 1:25){
  ...
  intr_rate=intvec[k]
  P2_rate=p2vec[k]
  t = iqtree[(iqtree$pop_size==pop_size & iqtree$rec_rate==rec_rate & iqtree$mut_rate==mut_rate & iqtree$intr_rate==intr_rate & iqtree$P2_rate==P2_rate & iqtree$align_length==align_length),]
  
  threshold = 0.001
  t_sig = iqtree[(iqtree$pop_size==pop_size & iqtree$rec_rate==rec_rate & iqtree$mut_rate==mut_rate & iqtree$intr_rate==intr_rate & iqtree$P2_rate==P2_rate & iqtree$align_length==align_length & iqtree$p_value<threshold),]
  t_nonsig = iqtree[(iqtree$pop_size==pop_size & iqtree$rec_rate==rec_rate & iqtree$mut_rate==mut_rate & iqtree$intr_rate==intr_rate & iqtree$P2_rate==P2_rate & iqtree$align_length==align_length & iqtree$p_value>=threshold),]
  ...
  #legend("topright", bty="n" ,legend=paste0("int_rate = ", intr_rate, "\n" ,"P2-rate = ",P2_rate ))        
