...
# Make sure that the focal trio can be parsed.
if focal_trio
	focal_spcs = focal_trio.split(",")
	unless focal_spcs.size == 3
		puts "ERROR: Expected three comma-separated species in the focal trio string!"
		exit 1
	end
end
...
taxa = []
trees.each do |t|
	taxa_this_tree = t.scan(/[A-Za-z_0-9][A-Za-z_0-9]+/)
...
# Make sure that the focal trio is included in the list of taxa.
if focal_trio
	focal_spcs.each do |focal_spc|
		unless sorted_taxa.include?(focal_spc)
			puts "ERROR: Focus species #{focal_spc} is not included in the present list of taxa!"
			exit 1
		end
	end
end

# Remove all taxa except the focal taxon from the list of taxa.
if focal_trio
	sorted_taxa = focal_spcs
end
	...
	(x+1).upto(sorted_taxa.size-1) do |y|
		...
		(y+1).upto(sorted_taxa.size-1) do |z|
			...
			if focal_trio
				...
				if n_taxa23_monophyletic + n_taxa13_monophyletic == 0
					d = 0
				else
					d = (n_taxa23_monophyletic - n_taxa13_monophyletic)/(n_taxa23_monophyletic + n_taxa13_monophyletic).to_f
				end

				# Test if the imbalance between discordant trees is greater than expected by chance.
				pr = 0
				n = n_taxa23_monophyletic + n_taxa13_monophyletic
				k = (n_taxa13_monophyletic+1)
				k.times do |i|
				...
				outstring = ""
				outstring << "#{taxon1.ljust(10)}#{taxon2.ljust(10)}#{taxon3.ljust(10)} #{n_taxa12_monophyletic.to_s.rjust(8)}  #{n_taxa23_monophyletic.to_s.rjust(8)}  #{n_taxa13_monophyletic.to_s.rjust(8)} #{('%.3f' % d).rjust(8)}  #{('%.8f' % pr).rjust(12)}\n"
			...
			# Stop if a focal trio was specified.
			if focal_trio
				exit 0
			end
		end
