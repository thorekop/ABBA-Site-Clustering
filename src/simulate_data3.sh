	...
	do
		...
		do
			...
			do
			    ...
			    # Set log file.
			    log=../log/msprime/pop_size_${pop_size}_div_time_${divergence_time}_rec_rate_${rec_rate}_mut_rate_${mut_rate}_intr_rate_${intr_rate}_P2_rate_${P2_rate}_r${r}.3.log
			    ...
			    then
				rm -f ${log}
				sbatch --account ${acct} -o ${log} simulate_data3.slurm ${prefix} ${pop_size} ${divergence_time} ${rec_rate} ${mut_rate} ${intr_rate} ${P2_rate} ${seed}
			    fi
			done
		    done
		done
