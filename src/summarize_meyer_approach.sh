...
# Write headerline.
echo -e "pop_size\tdiv_time\trec_rate\tmut_rate\tintr_rate\tP2_rate\talign_length\trun\tmrca_reduction\ttopo_ok" > ../res/tables/meyer_approach.txt

# Take iqtree file IDs.
for align_length in 200 500 1000
do
    for file in ../res/meyer_et_al/*/beast2_${align_length}.mrca_reductions_all_max.txt
    do
	...
	then
        ...
        echo -e "${pop_size}\t${div_time}\t${rec_rate}\t${mut_rate}\t${intr_rate}\t${P2_rate}\t${align_length}\t${run}\t${mrca_reduction_AB}\t${topo_ok}" >> ../res/tables/meyer_approach.txt
    done
done
