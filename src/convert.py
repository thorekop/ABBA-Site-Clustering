# m_matschiner Thu Dec 9 00:45:13 CET 2021
...
parser = argparse.ArgumentParser(
    ...
    -----------------------------------------
      ...
      Accepted output formats: phylip, fasta, nexus, genepop,
       ...
       biallelic SNPs), vcf, vcf_bi, dists (a distance
       matrix).
    '''))
parser.add_argument(
    '-v', '--version',
...
elif output_format == "dists":
    max_id_length = 0
    for record_id in record_ids:
        if len(record_id) > max_id_length:
            max_id_length = len(record_id)
    output_string += str(len(record_ids)) + "\n"
    for x in range(len(record_ids)):
        output_string += record_ids[x].ljust(max_id_length)
        for y in range(len(record_ids)):
            n_different = 0
            n_identical = 0
            for pos in range(len(record_seqs[0])):
                if record_seqs[x][pos] in ["A", "C", "G", "T"] and record_seqs[y][pos] in ["A", "C", "G", "T"]:
                    if record_seqs[x][pos] == record_seqs[y][pos]:
                        n_identical += 1
                    else:
                        n_different += 1
            if n_different + n_identical > 0:
                output_string += "  " + '{:.3f}'.format(n_different/(n_different+n_identical))
            else:
                output_string += "     NA"
        output_string += "\n"
