...
do
    for vcf in ../res/msprime/pop_size_${pop_size}_*vcf.gz
    do
	...
	do
	    ...
	    # Set the prefix.
	    table=../res/alignments/${vcf_id}_${length}_pi_sites.txt
		...
		then
		    ...
		    # Set the log file.
		    log=../log/alignments/${vcf_id}_${length}_pi_sites.txt
		    rm -f ${log}
		    
		    # Run slurm script.
		    sbatch --account ${acct} -o ${log} get_number_of_pi_sites.slurm ${vcf} ${table} ${number} ${length}
		fi
	    fi
	done
    done
